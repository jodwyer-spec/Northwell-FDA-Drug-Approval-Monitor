...
function getApprovalLetterUrl(sub) {  
  ...
  for (var i = 0; i < docs.length; i++) {  
    if (docs[i].type && docs[i].url) {  
      if (docs[i].type.toLowerCase() === "letter") {  
        return docs[i].url;  
      }  
    }  
  }  
  ...
  for (var i = 0; i < approvals.length; i++) {
    ...
    for (var c = 0; c < row.length; c++) {
      ...
      xml += '<Cell ss:StyleID="' + styleToUse + '"><Data ss:Type="String">' +
        ...
        if (letterUrl) {  
      xml += '<Cell ss:StyleID="' + normalStyle + '" ss:HRef="' + escapeXml(letterUrl) + '"><Data ss:Type="String">View Letter</Data></Cell>\n';  
    } else {  
